...
```ts
import { IconMarker } from '@fitx/components';
// <icon-marker />
```
or
```ts
import Marker from '@fitx/components/icons/marker.svg?url';
// <img :src="Marker" alt="">
```

Icons are located in `assets/icons/` and can be automatically imported as component via command line
```bash
npx tsx ./utils/create-icons-export.ts
```
...
### Example Code for icon-marker-x
```xml
